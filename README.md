...
# 上传核心文件（从 PC 执行）
scp bt_engine.py reminder_nodes.py reminder_tree.py groot2_server.py \
    cat@192.168.1.107:~/reminder_bt/
```
...
```bash
# 立即触发
curl -s -X POST http://127.0.0.1:8000/api/reminders \
...
```bash
curl -s http://127.0.0.1:8000/api/reminders | python3 -m json.tool
```

### 查看行为树状态（需要 BehaviorTreeMonitor）
...
```
Web 浏览器 → :8001 API → reminders.json
                            ↓
                   groot2_server (port 1669)
                            ↓
                   行为树 (13 nodes)
                   ReactiveSequence
                   ├── CheckTime
                   └── Sequence
                       ...
                       ├── PlaySpeaker → robot_audio_node → 喇叭
...
| `pc_frontend/index.html` | PC 本地前端页面 |