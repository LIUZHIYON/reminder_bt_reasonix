...
4. 成功则 UPDATE `ai_pet_reminders` SET `delivery_id`、`delivery_params`、`status=sent`、`sent_time=now()`
5. 失败（如设备离线）则保持 `status=pending`，返回 msg 说明下发失败原因
...
> **⭐ 新增定时传话字段：** 传话支持定时发送（`scheduledTime`）和重复模式（`repeatType`）。`scheduledTime` 通过 WebSocket 随消息下发给设备，**由设备端自行计时执行**，服务端不参与定时调度。详见 [§23.3](#233-新增传话消息创建即下发)。
...
| repeat_type | string? | 否 | 按重复类型筛选：none / daily / weekly / monthly |
...
| scheduledTime | datetime? | ⭐新增 定时发送时间（不为空且大于当前时间时不立即下发） |
| repeatType | string? | ⭐新增 重复类型：none / daily / weekly / monthly，默认 none |
...
| scheduledTime | datetime | 否 | ⭐新增 定时发送时间（随消息下发给设备，**由设备端自行计时**） |
| repeatType | string | 否 | ⭐新增 重复类型：none / daily / weekly / monthly，默认 none |

**说明:** `petId`、`userId`、`createBy` 由服务端自动填充。创建后**立即下发到设备**，`scheduledTime` 和 `repeatType` 随 WS 消息传给设备端，由设备端自行判断计时执行。服务端不参与定时调度。

**下发流程：**
1. 校验内容格式（text 模式 ≤200 字符；voice 模式需 mediaUrl）
2. INSERT `ai_pet_relay_messages`（status=`pending`）
3. 通过 `build_relay_params` 构建 `delivery_params`（包含 `scheduled_time`、`repeat_type` 等，随消息下发）
4. 通过 `send_relay_message_to_aipet()` 下发 WS `relay_message_delivery` 消息到设备
5. 成功则 UPDATE `ai_pet_relay_messages` SET `delivery_id`、`delivery_params`、`status=sent`、`sent_time=now()`
6. 失败（如设备离线）则保持 `status=pending`，返回 msg 说明下发失败原因
