...
```json
{
  ...
  "scheduled_time": "2026-06-26T08:00:00",
  "repeat_type": "none",
  "priority": "high",
  "timestamp": "2025-01-15T12:30:00.123456",
  "timeout": 30
...
| scheduled_time | string? | ⭐新增 ISO8601 格式，定时发送时间（null 表示立即播报，**由设备端自行计时**） |
| repeat_type | string | ⭐新增 重复类型：none / daily / weekly / monthly，默认 none |
| priority | string | 优先级（默认 high） |
| timestamp | string | ISO 8601 时间戳 |
| timeout | int | 超时秒数（默认 30） |
